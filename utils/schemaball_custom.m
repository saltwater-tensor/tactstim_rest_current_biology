function h = schemaball_custom(r, lbls, ccolor, ncolor, szdata,colr_mat_2_orig,figstring,output_path)
...
% Ccolor
% ccolor = [0 0 0]; 
% ccolor = [1,1,1;0,0,0]; % UNCOMMENT FOR BLACK LINES 
...
% darkest are the low values and brightest are the increasingly large
N2        = 2*N;
[n, isrt] = histc(r(tf), linspace(0,1 + eps(100),N2 + 1));
% [n, isrt] = histc(r(tf), linspace(0,1 + eps(100),N2 + 1));
...
% figstring = [output_path figstring];
% C = gcf;
% savefig(C,figstring)
