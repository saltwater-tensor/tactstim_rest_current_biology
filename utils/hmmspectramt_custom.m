function fit = hmmspectramt_custom(data,T,Gamma,options)
%
...
% Spectral estimation parameters, stays constant across all subject
...
Nf = length(f); options.Nf = Nf;
 ...
 % check tapers
tapers=dpsschk(options.tapers,options.win,Fs);
ntapers = options.tapers(2);

% Calculate statewise 
% Remeber that the data and T sent to this function is either all the data
% concatenated across all subjects or subject by subject so there is no
% averaging across subjects
for k=1:K
           
    % Multitaper Cross-frequency matrix calculation
    if options.p > 0 % For calculation of p-value
    ...
    for n = 1:length(T) % Doing trial wise
        if iscell(data)
            ...
            X = data((1:TT(n)) + sum(TT(1:n-1)) , : ); %get trial wise original data
            LT= 1;
        end
        t0 = 0;
        for nn = 1:LT % elements for this subject
            ...
            try
                % Checking for small state visits and gamma masking for k    
